...
io.on('connection', (socket) => {
  ...
  if (sessionId && userId && role && stationId && displayName) {
    ...
    socket.on('VOICE_OFFER', (data) => {
      console.log(`📞 Voice offer from ${userId} to session ${sessionId}`);
      const session = sessions.get(sessionId);
      if (session) {
        // Enviar offer para o outro participante
        const targetUser = Array.from(session.participants.values()).find(p => p.userId !== userId);
        if (targetUser && targetUser.socketId) {
          io.to(targetUser.socketId).emit('VOICE_OFFER_RECEIVED', {
            offer: data.offer,
            fromUserId: userId,
    ...
    socket.on('VOICE_ANSWER', (data) => {
      console.log(`📞 Voice answer from ${userId} to session ${sessionId}`);
      const session = sessions.get(sessionId);
      if (session) {
        // Enviar answer para o outro participante
        const targetUser = Array.from(session.participants.values()).find(p => p.userId !== userId);
        if (targetUser && targetUser.socketId) {
          io.to(targetUser.socketId).emit('VOICE_ANSWER_RECEIVED', {
            answer: data.answer,
            fromUserId: userId,
            fromUserName: displayName
          });
    ...
    socket.on('VOICE_ICE_CANDIDATE', (data) => {
      console.log(`🧊 ICE candidate from ${userId} to session ${sessionId}`);
      const session = sessions.get(sessionId);
      if (session) {
        // Enviar ICE candidate para o outro participante
        const targetUser = Array.from(session.participants.values()).find(p => p.userId !== userId);
        if (targetUser && targetUser.socketId) {
          io.to(targetUser.socketId).emit('VOICE_ICE_CANDIDATE_RECEIVED', {
    ...
    socket.on('VOICE_CONNECTION_STATE', (data) => {
      console.log(`🔗 Voice connection state from ${userId}: ${data.state}`);
      const session = sessions.get(sessionId);
      if (session) {
        // Notificar estado da conexão para o outro participante
        const targetUser = Array.from(session.participants.values()).find(p => p.userId !== userId);
        if (targetUser && targetUser.socketId) {
          io.to(targetUser.socketId).emit('PARTNER_VOICE_STATE', {
            state: data.state,
            fromUserId: userId,
            fromUserName: displayName
          });
        ...
        // Broadcast para ambos sobre o estado geral da chamada
        session.participants.forEach(participant => {
          if (participant.socketId) {
            io.to(participant.socketId).emit('VOICE_SESSION_UPDATE', {
              sessionId: sessionId,
